...
public class MasterMind
{
	...
	public static void playGame(int numberOfColors, int numberOfPits)
	{
		...
		long temp = 0;
		for(int i = 0; i < 10; i++){
			temp = playGameOnce(numberOfColors,numberOfPits);
			System.out.println("Time for run with "+numberOfColors+" colors and "+
				numberOfPits+" pits is : "+temp+" ns");
			average += temp;
		}
		System.out.println("Average time for 10 runs with "+numberOfColors+" colors and "+
				numberOfPits+" pits is : "+average/10+" ns");
	...
	//	elimination(guess_result, guess,numberOfColors,numberOfPits);
		...
		while(guess_result[0] != numberOfPits)
		{
			...
			output_result(guess_result);
				/*if(guess_result[0] == 0 && guess_result[1] == 0){
					System.out.println("before set size="+S.size());
					S.clear();
	...
	/*private static Vector<int[]> newset(int[] guess2, int numberOfPits) {
		...
		for(int i1 = 0; i1 < size; i1++)
		{
			for(int i2 = 0; i2 < size; i2++)
			{
				for(int i3 = 0; i3 < size; i3++)
				{
					...
					{
						int[] a = {colors.get(i1),colors.get(i2),colors.get(i3),colors.get(i4)};
						vector.add(a);
					}
				}
			}
